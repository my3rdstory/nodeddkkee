...
# # 토르 설치 및 설정
# echo "토르 설치 및 설정을 시작합니다..."

# # Bitcoin Core 서비스 중지
# echo "Bitcoin Core 서비스 중지 중..."
# sudo systemctl stop bitcoind
...
#     if ! systemctl is-active --quiet bitcoind; then
#         echo "Bitcoin Core 서비스가 정상적으로 중지되었습니다."
#         break
#     else
#         echo "서비스 중지 대기 중... (시도 $STOP_ATTEMPT/$MAX_STOP_ATTEMPTS)"
#         sleep 2
#         STOP_ATTEMPT=$((STOP_ATTEMPT+1))
#     fi
# done

# if [ $STOP_ATTEMPT -gt $MAX_STOP_ATTEMPTS ]; then
#     error_exit "Bitcoin Core 서비스 중지 시간 초과"
# fi

# # Bitcoin Core에 토르 설정 추가
# echo "Bitcoin Core에 토르 설정 추가 중..."
# cat >> ${USER_HOME}/.bitcoin/bitcoin.conf << EOF
...
#     if systemctl is-active --quiet bitcoind; then
#         echo "Bitcoin Core 서비스가 정상적으로 시작되었습니다."
#         break
#     else
#         echo "서비스 시작 대기 중... (시도 $START_ATTEMPT/$MAX_START_ATTEMPTS)"
#         sleep 2
#         START_ATTEMPT=$((START_ATTEMPT+1))
#     fi
# done

# if [ $START_ATTEMPT -gt $MAX_START_ATTEMPTS ]; then
#     echo "서비스 로그 확인:"
#     sudo journalctl -u bitcoind --no-pager | tail -n 20
#     error_exit "Bitcoin Core 서비스 시작 시간 초과"
# fi
...
#     curl https://deb.torproject.org/torproject.org/A3C4F0F979CAA22CDBA8F512EE8CBC9E886DDD89.asc | sudo gpg --dearmor | sudo tee /usr/share/keyrings/tor-archive-keyring.gpg >/dev/null
# fi
...
# while [ $TOR_CHECK -le $MAX_TOR_CHECK ] && [ "$TOR_READY" = "false" ]; do
#     echo "토르 서비스 상태 확인 중... (시도 $TOR_CHECK/$MAX_TOR_CHECK)"
#     if sudo systemctl is-active --quiet tor; then
#         echo "토르 서비스가 정상적으로 실행 중입니다."
#         if nc -z localhost 9050; then
#             echo "토르 SOCKS 포트(9050)가 열려있습니다."
#             TOR_READY=true
#         else
#             echo "토르 SOCKS 포트를 기다리는 중..."
#         fi
#     else
#         echo "토르 서비스가 아직 시작되지 않았습니다."
#         sudo systemctl status tor --no-pager | head -n 20
#     fi
    
#     if [ "$TOR_READY" = "false" ]; then
#         if [ $TOR_CHECK -lt $MAX_TOR_CHECK ]; then
#             echo "5초 후 다시 확인합니다..."
#             sleep 5
#         fi
#         TOR_CHECK=$((TOR_CHECK+1))
#     fi
# done
...
#     error_exit "토르 서비스 시작 실패"
# fi

# # 토르 연결 테스트 및 JSON 파일 생성
# echo "토르 연결 테스트 중..."
...
#         echo "토르 연결 테스트 성공: 토르 네트워크를 통해 정상적으로 접속됩니다."
        
#         # JSON 파일 생성
#         cat > "${USER_HOME}/tor.json" << EOF
# {
#     "tor_status": "connected",
#     "created_at": "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
# }
# EOF
#         chown ${USER_NAME}:${USER_NAME} "${USER_HOME}/tor.json"
#         chmod 600 "${USER_HOME}/tor.json"
#         echo "토르 상태가 ${USER_HOME}/tor.json 파일에 저장되었습니다."
#     else
#         echo "경고: 토르 연결 테스트 실패. 토르가 제대로 작동하지 않을 수 있습니다."
#     fi
# else
#     echo "경고: curl이 설치되어 있지 않아 토르 연결 테스트를 수행할 수 없습니다."
# fi

# echo "토르 설치 및 설정이 완료되었습니다."
# echo "tail -f ~/.bitcoin/debug.log 명령어로 로그를 확인할 수 있습니다."